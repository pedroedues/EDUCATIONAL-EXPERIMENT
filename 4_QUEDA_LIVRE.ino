void quedaLivre(String metodoControle) {
  ...
  while (QUEDA_OPERACAO == HIGH) {
    ...
    //ATENCAO, PROVAVELMENTE FALTA ALGO AQUI, PULEI DIRETO PARA PROGARAMACAO DO APP, LEMBRAR DE VERIFICAR DEPOIS QUANDOESTIVER COM O TECLADO EM MAOS
    if (metodoControle == "teclado" && posicao == HIGH) {
      ...
      switch (decisao) {
        ...
        case '3':
          ...
          if (resultado == "new") quedaLivre("teclado");//USUARIO SOLICITOU NOVA MEDICAO
          else calculoQueda(tempoFinal, "lcd");//SIGNIFICA QUE SAIU DO WHILE COM O VALOR DE CERTO, PORTANTO, EXIBIRA OS CALCULOs
          break;
      }
      ...
      while (bt.read() != 1 && bt.read() != 89 ) {
        if (bt.available()) if (bt.read() == 89) resetarArduino(); //TLVZ TENHAMOS PROBLEMAS AO IMPLEMENTAR O TECLADO. CASE DE, PENSAR EM COLOCAR LOW NO QUEDA_OPERACAO PARA ELE SAIR DO WHILE E VOLTAR PRA ONDE A FUNCAO QUEDALIVRE() FOI CHAMADA
      }
      lcdContagem();//3, 2, 1, PIIIII
      digitalWrite(eletroIma, LOW);//SOLTA MOVEL
...
void calculoQueda(float tempo, String controle) {
  ...
  if (controle == "lcd") {
    ...
    if (decisao == '2') quedaLivre("teclado");
