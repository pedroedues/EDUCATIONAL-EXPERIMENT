...
void loop() {
  ...
  switch (decisao) {
    ...
    case '3'://DEVMODE
      ...
      while (resultado != "certo") {//SAI DO WHILE QUANDO A FUNCAO "senha()" RETORNA "certo"
...
void testeTeste() {
  ...
  switch (decisao2) {
    case '1': planoInclinado("teclado");//FUNCOES PLANO INCLINADO
      break;

    case '2': quedaLivre("teclado");//FUNCOES QUEDA LIVRE
      break;

    case '3': energiaCinetica("teclado");//FUNCOES ENERGIA CINETICA
      break;
...
char decisaoControle() { //CHAMA TECLADO PARA SABER SE O USUARIO VAI CONTROLAR FISICAMENTE OU VIA PP. 'A' - app, 'B' - experimento
  char decisao = '@';
  int i = 0;
  lcd.clear();
  lcd.setCursor(2, 0);
...
char decisaoExperimento() {//DECISAO DE EXPERIMENTO (QUANDO CONTROLADO FISICAMENTE). 1 - PLANO, 2 - QUEDA, 3 - ENERGIA
  char decisao;
  int i = 0;
  lcd.clear();
  lcd.setCursor(0, 0);
...
void app() {
  ...
  while (i == 0) {
    //if (bt.available())
    //decisao = bt.read();
    switch (bt.read()) {//CASO DE PROBLEMA, ZERAR VARIAVEL APOS CHAMADA DO EXPERIMENTO
      case 'P':
...
void alertaMov(String tipo) { //ALERTA PARA MOVIMENTACAO DO PLANO E RECEBE SE O AVISO É PELO APP OU LCD
  if (tipo == "lcd") {
  ...
  if (ang == 0 || ang == 90) {
    switch (ang) {
      case 0:
        ...
        while (digitalRead(chaveLimite0) != LOW) { //ENQUANTO A  CHAVE FIM DE CURSO N ESTIVER ACIONADA, O MOTOR RECEBE OS PULSOS E SE MOVIMENTA. QUANDO PRESSIONADA, CHAVE RETORNA LOW
      ...
      case 90:
        ...
        digitalWrite(enaPin, LOW);//ESSA LINHA HABILITA A FUNÇÃO HALFSTEP, ENT DA UM PASSO DE 0.9º
        while (i != 8000) { //ENQUANTO A  CHAVE FIM DE CURSO N ESTIVER ACIONADA, O MOTOR RECEBE OS PULSOS E SE MOVIMENTA. QUANDO PRESSIONADA, CHAVE RETORNA LOW
          digitalWrite(stepPin, HIGH);
    ...
    for ( int x = 0; x <= ppr ; x++) {
      digitalWrite(stepPin, HIGH);
      delayMicroseconds(800); //PROVAVELMETE ESSE INSTANTE ESTEWJA ALTO DEMAIS
      digitalWrite(stepPin, LOW);
      delayMicroseconds(800);
    }
    digitalWrite(enaPin, HIGH);
...
boolean verificaEletro(String controle) {//VERIFICA SE O MOVEL ESTA NO ELETROIMA E RECEBE SE O AVISO DEVE SER PELO APP OU LCD
  boolean posicao = LOW;
  posicao = digitalRead(sensor);
  while (digitalRead(sensor) != LOW) {
    if (bt.available()) if (bt.read() == 'Y') resetarArduino();

    /* else if (digitalRead(sensor) == HIGH && controle == "teclado") {//SE O SENSOR NAO VER NADA NO ELETROIMA E O AVISO TIVER QUE SER FEITO NO LCD
       //AVISO NO LCD
       //lcd.print("PRENDA O OBJETO ");
       posicao = LOW;
      } else if (digitalRead(sensor) == LOW) posicao = HIGH;//ENXERGOU O MOVEL NO ELETROIMA*/
...
int decisaoNum(char experimento) {
  ...
  if (i == 1 && tecla[1] == 100) { //SE O USUARIO DIGITAR APENAS UM NUMERO E CONFIRMAR COM #, NESTE CASO, # EH IGUAL A 100
    num = tecla[0];
    return num;
...
void lcdContagem() { //3, 2, 1 PARA LIBERACAO DO MOVEL
  lcd.clear();
  lcd.setCursor(0, 0);
...
int marcaInstantes(char experimento, String controle, float angReal) {
  ...
  while (i <= 4) {
    if ((i == 0) && digitalRead(sensor1) == LOW) {//SENSOR UTILIZADO RETORNA LOW SEMPRE QUE IDENTIFICA UM OBJETO
      temp = millis();
      //teste();
      i += 1;
    }
    ...
    /*if ((i == 3) && (digitalRead(sensor4) == LOW)) {
      tempo[2] = millis()  - temp;
      //teste();
      i += 1;
      }*/
  ...
  if (controle == "app") {
    //IMPRIMIR DADOS NO APP
    int i = 0;
    /*while (i == 0) {
      if (bt.available()) {
        int acao = bt.read();//RECEBE 1, SAI DO WHILE E PERMITE A EXIBICAO DOS INSTANTES
        if (acao == 1) i++;
      }
      }*/
    bt.print("\t\tT1: ");
    bt.print(tempo[0]); bt.println(" ms");
    bt.print("\t\tT2: ");
    bt.print(tempo[1]); bt.println(" ms");
    bt.print("\t\tT3: ");
    bt.print(tempo[2]); bt.println(" ms");
    bt.print("\t\tT4: ");
    bt.print(tempo[3]); bt.println(" ms");
    lcd.clear();
    for (i = 0; i < 4 ; i++) {
      lcd.setCursor(0, i);
      lcd.print("T");
    ...
    /*if (experimento == 'P' || experimento == 'E') {//SE O EXPERIMENTO FOR O PLANO INCLINADO ELE MOSTRA O ANGULO ALCANCADO PELO SISTEMA
      lcd.print("  ");
      lcd.print(angReal);
      lcd.print("º");
      Serial.println("Angulação obtida: ");
  ...
  Serial.print("\n\n\tMedições");
  Serial.print("\nInstante 1 = ");
  Serial.print(tempo[0]); Serial.print(" ms");
  Serial.print("\nInstante 2 = ");
  Serial.print(tempo[1]); Serial.print(" ms");
  Serial.print("\nInstante 3 = ");
  Serial.print(tempo[2]); Serial.print(" ms");
  Serial.print("\nInstante 4 = ");
  Serial.print(tempo[3]); Serial.println(" ms");
  return tempo[3];
}



String senha(char experimento) {//RECEBE O EXP QUE ESTA SENDO USADO
  char password[20],
       teclaPressionada;
  ...
  /*switch (experimento) {
    case 'P':
      if ( i == 5 && (password[0] == 'A' && password[1] == 'B' && password[2] == '1' && password[3] == '2' && password[4] == '3' )) {//SENHA PLANO INCLINADO = AB123
        lcd.setCursor(3, 1);
        lcd.print("SENHA CORRETA!");
        delay(300);
        return "certo";
      }
      else if ((password[0] != 'A' || password[1] != 'B' || password[2] != '1' || password[3] != '2' || password[4] != '3' )) {
        lcd.setCursor(2, 1);
    ...
    case 'Q':*/
  if ( i == 5 && (password[0] == 'B' && password[1] == 'A' && password[2] == '3' && password[3] == '2' && password[4] == '1' )) {//SENHA QUEDA LIVRE = BA321
    lcd.setCursor(3, 1);
    lcd.print("SENHA CORRETA!");
    delay(300);
    return "certo";
  ...
  /*break;
    case 'E':
    if ( i == 5 && (password[0] == 'C' && password[1] == 'A' && password[2] == '7' && password[3] == '8' && password[4] == '9' )) {//SENHA ENERGIA CINETICA = CA789
    lcd.setCursor(3, 1);
    lcd.print("SENHA CORRETA!");
    delay(300);
    return "certo";
    }
    if ((password[0] != 'C' || password[1] != 'A' || password[2] != '7' || password[3] != '8' || password[4] != '9' )) {
    lcd.setCursor(2, 1);
    lcd.print("SENHA INCORRETA!");
    return "errado";
    }
    ...
    case 'T':
    if ( i == 5 && (password[0] == '0' && password[1] == '8' && password[2] == '0' && password[3] == '0' && password[4] == 'A' )) {//SENHA TESTE = 0800A
    lcd.setCursor(3, 1);
    lcd.print("SENHA CORRETA!");
    delay(100);
    return "certo";
    }
    if ((password[0] != '0' || password[1] != '8' || password[2] != '0' || password[3] != '0' || password[4] != 'A' )) {
    lcd.setCursor(2, 1);
    lcd.print("SENHA INCORRETA!");
    return "errado";
    }
...
char medouMenu(String controle) {
  ...
  if (controle == "teclado") {
    ...
    while (decisao != 'Z' && decisao != 'Y') {
      ...
      if (bt.available()) decisao = bt.read();
    }
  }
  digitalWrite(13, LOW);
  return decisao;
...
void developerMode() {//MODO PARA TESTE DE COMPONENTES
...
void reset() {
  while (true) ;//fica aqui até
}

void teste() {//USADA PARA TESTAR, VER AONDE O CODIGO ESTA OU OUTROS
  digitalWrite(13, HIGH);
  delay(100);
  digitalWrite(13, LOW);
}
