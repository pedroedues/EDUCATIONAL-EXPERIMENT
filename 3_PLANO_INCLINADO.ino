...
void planoInclinado(String metodoControle)
{
  ...
  while (PLANO_OPERACAO == HIGH)
  {
    ...
    if (metodoControle == "teclado" && posicao == HIGH)
    {
      ...
      while (ang == 0 || ang > 80)
      { //VERIFICAR - ESTA ESTRANHO - como que ele entra no proximo if se enquanto foir maior que 80 ele rodara este if while
        lcd.setCursor(2, 0);
        lcd.print("PLANO INCLINADO");
        ang = decisaoNum('P'); //PEGA O VALOR LIDO PELO TECLADO
        if (ang == 2089)
        { //VALOR RETORNADO AO  PRESSIONAR '*', O QUAL TEM A FUNCAO DE VOLTAR, LOGO, RESETAR ARDUINO
          resetarArduino();
        }
      }
      alertaMov("lcd");
      ...
      case '3':
        ...
        if (resultado == "new")
          planoInclinado("teclado"); //USUARIO SOLICITOU NOVA MEDICAO
        else
        {
        ...
        while (j == 0)
        {
          if (bt.available())
          {
            ...
            if (liberar == 1)
              j++;
            if (liberar == 89)
              resetarArduino(); //89 eh o valor do 'Y' (ascII), o qual sera enviado sempre que clicar para voltar, ao clicar para voltar o arduino eh resetado
          }
        }
        lcdContagem();                                        //3, 2, 1, PIIIII
        digitalWrite(eletroIma, LOW);                         //SOLTA MOVEL
