void energiaCinetica(String metodoControle) {
  ...
  while (ENERGIA_OPERACAO == HIGH) {
    ...
    /*while (posicao == LOW) { //ENQUANTO NAO ESTIVER NADA PRESO NO ELETROIMA
      if (metodoControle == "teclado") posicao = verificaEletro("teclado"); //VERIFICA SE EXISTE ALGO PRESO NO ELETROIMA
    ...
    if (metodoControle == "teclado") {
      ...
      while ( ang == 0 || ang > 90) {
        ...
        if (ang == 2089) { //VALOR RECEBIDO QUANDO USUARIO QUISER VOLTAR PARA O MENU
          resetarArduino();
        }
      }
      ...
      while ( massa == 0 || massa > 250) {
        ...
        if (massa == 2089) { //VALOR RECEBIDO QUANDO USUARIO QUISER VOLTAR PARA O MENU
          resetarArduino();
        }
      }
      ...
      lcd.setCursor(3, 3);
      lcd.print("'1' - Liberar");
      while (teclado.getKey() != '1') {}//FICA AQUI ATE O USUARIO PRESSIONAR 'A' PARA LIBERAR
      ...
      switch (decisao) {
        ...
        case '3':
          ...
          if (resultado == "new") energiaCinetica("teclado");//USUARIO SOLICITOU NOVA MEDICAO
          else calculoEnergia(angReal, massa, tempoFinal, "lcd");//SIGNIFICA QUE SAIU DO WHILE COM O VALOR DE CERTO, PORTANTO, EXIBIRA OS CALCULOs
          break;
      }
    }
...
void calculoEnergia(float angReal, float massa, int tempo, String controle) {
  ...
  /*ATENÇÃOOOOOOOOOOOOO PROGRAMAR UMA TELA PARA
    FALAR DAS UNIDADES UTILIZADAS, FORMULAS, VALOR
    DA GRAVIDADE E TLVZ OUTRA COISA QUE EU N ME
    LEMBRO AGORA*/
  lcd.clear();
  ...
  if (controle == "lcd") {
    ...
    if (decisao == '2') energiaCinetica("teclado");
